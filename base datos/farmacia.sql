...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 30-05-2022 a las 18:58:54
...
INSERT INTO `login` (`IdUsuario`, `Contraseña`) VALUES
('user2', '{Orė/?');
...
CREATE TABLE `pago_producto` (
  `IdUsuario` varchar(10) NOT NULL,
  `IdCodigo` int(12) DEFAULT NULL,
  `Nombre_p` varchar(20) DEFAULT NULL,
  `Precio_p` int(3) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
INSERT INTO `productos_medicos` (`Nombre_Producto`, `IdCodigo`, `Descripcion`, `Numero_Cantidad`) VALUES
...
ALTER TABLE `pago_producto`
  ADD PRIMARY KEY (`IdUsuario`),
  ADD KEY `IdUsuario` (`IdUsuario`,`IdCodigo`),
...
--
ALTER TABLE `pago_producto`
  ADD CONSTRAINT `pago_producto_ibfk_1` FOREIGN KEY (`IdUsuario`) REFERENCES `login` (`IdUsuario`) ON UPDATE CASCADE,
  ADD CONSTRAINT `pago_producto_ibfk_2` FOREIGN KEY (`IdCodigo`) REFERENCES `productos_medicos` (`IdCodigo`) ON UPDATE CASCADE;
COMMIT;
